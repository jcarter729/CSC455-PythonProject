    ...
    def start(self):
        ...
        try:
            ...
            while self.running:
                ...
                # Check if both windows exist (means connection is working)
                try:
                    local_visible = cv2.getWindowProperty("You (Local)", cv2.WND_PROP_VISIBLE) >= 0
                    remote_visible = cv2.getWindowProperty("Remote (Them)", cv2.WND_PROP_VISIBLE) >= 0
                    
                    if local_visible and remote_visible:
                        if not hasattr(self, 'connection_announced'):
                            print("🎉 Video call connected! Both video windows active.")
                            self.connection_announced = True
                    
                    # If windows are closed, quit
                    if local_visible and cv2.getWindowProperty("You (Local)", cv2.WND_PROP_VISIBLE) < 1:
                        break
                    if remote_visible and cv2.getWindowProperty("Remote (Them)", cv2.WND_PROP_VISIBLE) < 1:
                        break
                except:
                    pass
